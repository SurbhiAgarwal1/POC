...
export interface InstrumentationPackage {
  ...
  name: string;
  language: string;
  category: 'backend' | 'frontend' | 'infrastructure' | 'ai';
...
export const personas: Persona[] = [
  ...
  {
    id: "persona-ai-coder",
    name: "Prompting Dev (AI-First)",
    role: "AI-Augmented Software Engineer",
    avatar: "AI",
    experience: "3+ years using LLM code generation",
    quote: "I rarely read the official docs directly anymore. I copy the error into the LLM or ask it to generate my tracer code. But when it hallucinated standard headers, I spent two days debugging.",
    description: "An engineer representing the modern generation of developers who write code primarily by feeding instructions to AI assistants (Copilot, Cursor, LLMs). They suffer heavily when documentation schemas are poorly documented in public datasets, leading to hallucinations.",
    goals: [
      "Rapidly generate boilerplate setups using AI prompts.",
      "Debug configuration errors by pasting console outputs.",
      "Verify if code snippets generated by AI are using the latest version of the API."
    ],
    painPoints: [
      "LLMs frequently generate outdated or mixed v0.x and v1.x APIs.",
      "Lack of clean, machine-readable JSON schemas for LLMs to read and ground themselves on.",
      "Silent configuration failures that AI cannot easily diagnose because of lack of logs."
    ],
    tools: ["Cursor", "Copilot", "Claude 3.5 Sonnet", "ChatGPT API", "Vercel"],
    mentalModel: "Observability code is boilerplate logic that should be fully synthesizable by generative AI.",
    journeyStep: "Validating, debugging, and refactoring AI-generated telemetry code.",
    observabilityLiteracy: 60
  }
];
...
export const instrumentationPackages: InstrumentationPackage[] = [
  ...
  },
  {
    ...
    downloads: "28K/mo",
    stars: 340,
    description: "GenAI OpenTelemetry Instrumentation. Track LLM costs, prompts, token counts, temperature, and semantic configurations in OpenAI, Anthropic, and Langchain applications.",
    setupComplexity: "medium",
    signals: [
      {
        name: "gen_ai.client.token.usage",
        type: "metric",
        description: "Tokens consumed in requests split by type.",
        dataType: "Sum",
        unit: "tokens",
        attributes: [
          { name: "gen_ai.response.model", type: "string", description: "AI Model ID.", required: true, example: "gpt-4o" },
          { name: "gen_ai.token.type", type: "string", description: "Token category (prompt, completion).", required: true, example: "completion" }
        ]
      },
      {
        name: "gen_ai.chat.completion",
        type: "span",
        description: "A chat completion span capturing prompts and generations securely.",
        attributes: [
          { name: "gen_ai.request.model", type: "string", description: "Requested AI Model.", required: true, example: "claude-3-5-sonnet" },
          { name: "gen_ai.request.temperature", type: "double", description: "Temperature hyperparameter.", required: false, example: "0.7" },
          { name: "gen_ai.prompt", type: "string", description: "Scrubbed Prompt preview (if enabled).", required: false, example: "Write a React component..." },
          { name: "gen_ai.completion", type: "string", description: "Resulting generation response.", required: false, example: "Sure, here's..." }
        ]
      }
    ],
    configurations: [
      {
        name: "OTEL_GENAI_CAPTURE_PROMPTS",
        type: "boolean",
        defaultValue: "false",
        description: "Enables capturing raw text input prompts inside spans (high privacy risk).",
        environmentVariable: "OTEL_GENAI_CAPTURE_PROMPTS",
        importance: "critical"
      },
      {
        name: "OTEL_GENAI_TOKEN_PRICING_FILE",
        type: "string",
        defaultValue: "none",
        description: "Path to a local JSON detailing prompt pricing to automatically compute cost.",
        environmentVariable: "OTEL_GENAI_TOKEN_PRICING_FILE",
        importance: "optional"
      }
    ],
    supportedVersions: [
      { runtime: "Python >= 3.9", library: "openai >= 1.0.0" },
      { runtime: "Node.js >= 18.x", library: "openai >= 4.0.0" }
    ],
    dependencies: [
      "opentelemetry-semantic-conventions:0.4.0-alpha"
    ],
    coverageScore: 68
  }
];
...
export const aiScenarios: AiScenarios[] = [
  {
    prompt: "How do I trace my OpenAI completion calls and calculate billing costs?",
    assistantSummary: "To trace OpenAI completion requests and monitor pricing, you should deploy the `opentelemetry-instrumentation-openai` package. This instruments OpenAI completion APIs to generate a span named `gen_ai.chat.completion` representing the completion runtime, and exports a metric called `gen_ai.client.token.usage` which details token ingestion categorized by `prompt` and `completion` with standard `gen_ai.response.model` keys.",
    confidenceScore: 94,
    tokensUsed: 420,
    sources: [
      { title: "OTel Semantic Conventions: GenAI Client Rules (v1.25.0)", url: "https://opentelemetry.io/docs/specs/semconv/gen-ai/gen-ai-client/", confidence: 99 },
      { title: "opentelemetry-instrumentation-openai PyPI module", url: "https://pypi.org/project/opentelemetry-instrumentation-openai/", confidence: 92 }
    ],
    groundingAttributes: [
      { attribute: "gen_ai.client.token.usage", definition: "Sum metric of standard token usage.", isStandard: true },
      { attribute: "gen_ai.response.model", definition: "Representing the true model ID serving the completions.", isStandard: true },
      { attribute: "gen_ai.prompt", definition: "Captures user query prompts. Set OTEL_GENAI_CAPTURE_PROMPTS=true to activate.", isStandard: false }
    ],
    generatedCode: `from opentelemetry import tracer
from opentelemetry.instrumentation.openai import OpenAIInstrumentor
from openai import OpenAI

# Initialize instrumentation prior to calling client
OpenAIInstrumentor().instrument()

client = OpenAI()
response = client.chat.completions.create(
    model="gpt-4o",
    messages=[{"role": "user", "content": "Explain progressive disclosure in DX design."}]
)`
  },
  {
...
export const searchIndex = [
  ...
  { term: "LLM Hallucinations", category: "AI Discovery", link: "ai-section" },
